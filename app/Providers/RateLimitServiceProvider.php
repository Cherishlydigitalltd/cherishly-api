    ...
    public function boot(): void
    {
        ...
        RateLimiter::for('login', function (Request $request) {
            return Limit::perMinute(5)->by($request->ip())
                ->response(fn() => ApiResponse::error(
                    'Too many login attempts. Please try again in a minute.', null, 429
                ));
        });
        ...
        RateLimiter::for('otp', function (Request $request) {
            return Limit::perMinute(3)->by($request->ip())
                ->response(fn() => ApiResponse::error(
                    'Too many OTP requests. Please wait before trying again.', null, 429
                ));
        });
        ...
        RateLimiter::for('withdraw', function (Request $request) {
            return Limit::perHour(5)->by($request->user()?->id ?? $request->ip())
                ->response(fn() => ApiResponse::error(
                    'Too many withdrawal attempts. Please try again later.', null, 429
                ));
        });
    }
}
